...
import { ILoginBody, login, IRegistrationBody, register } from './authAPI';
...
export interface IAuthState {
  ...
  status: typeof IDLE | typeof PENDING | typeof LOGIN_SUCCESS| typeof LOGIN_FAILURE | typeof SERVER_ERROR
    | typeof REGISTER_SUCCESS | typeof REGISTER_FAILURE | typeof SOMETHING_BROKE;
}
...
export const registerAsync = createAsyncThunk(
  ...
  async (registrationBody: IRegistrationBody) => {
    const response = await register(registrationBody);
    // The value we return becomes the `fulfilled` action payload
    return response;
  }
);
